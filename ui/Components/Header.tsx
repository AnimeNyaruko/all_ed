	...
	useEffect(() => {
		...
		if (isAuthLoading && cachedAuthState === null) {
			...
			skeletonTimerRef.current = setTimeout(() => {
				if (isAuthLoading) {
					// Kiểm tra lại xem vẫn đang loading không
			...
			<nav className="px-6 py-4 container mx-auto">
				<div className="flex items-center justify-between">
					<Link href="/" className="space-x-2 flex items-center">
						<div className="w-10 h-10 relative flex items-center justify-center rounded-full border-3 border-solid border-[rgba(243,107,21,0.75)]">
							<Image alt="" src={logo} className="aspect-square w-3/4" />
						</div>
						<div className="text-2xl font-bold bg-gradient-to-r from-[#f12711] to-[#f5af19] bg-clip-text text-transparent">
							The AllEd
						</div>
					</Link>
				...
				<div
					ref={mobileMenuRef}
					className={`md:hidden mt-4 px-2 py-4 overflow-hidden border-t transition-all duration-300 ${
						isMobileMenuOpen
